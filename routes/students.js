...
router
	.get("/students", (req, res) => {
		...
		res.status(200);
		if (req.query.email) {
			let stud = students.getByEmail(req.query.email);
			if (stud) {
				res.json({ students: [stud], tot_students: 1 });
			} else {
				res.json({ students: [], tot_students: 0 });
			}
		} else {
	...
	.post("/students", (req, res) => {
		...
		if (email && firstname && lastname) {
			...
			if (stud) {
				...
				res.status(423);
				res.send(`Student with email=${email} has already registered. Use GET /students?email=${email} to get it`);
			}
		} else {
			res.status(400);
	...
	.put("/students/:studentID", (req, res) => {
		...
		if (id && email && firstname && lastname) {
			let result = students.updateStudent(id, req.body.email, req.body.firstname, req.body.lastname);
			if (result === 423) {
				res.status(423);
				res.send(`Student with email=${email} has already registered. Use GET /students?email=${email} to get it`);
			} else if (result === 200) {
