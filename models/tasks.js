...
function tasks_POST(req) {
    ...
    if (isString(result))
        return new Response(400, result, null);

    return new Response(201, null, result);
}
...
function tasks_taskID_PUT(req) {
    ...
    let status = tasks_list[id] ? "update" : "create";
    let result = createTask(id, req.body.question, req.body.answers);
